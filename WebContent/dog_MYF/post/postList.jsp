<%@ page import="dog_MYF.post.entity.Post"%>
					...
					<tbody>
						<c:forEach var="l" items="${list}">
							<tr>
								...
								<td class="title">
<%-- ADMIN --%>
									<p onClick="goDetailPage(${(user == 'admin') ? 'pDetailA' : 'pDetail'}, ${l.id})">
				...
				<tr>
					...
					<c:if test="${pNo > pages}">
						<td><input class="turn" type="button" value="◀"
							onClick="paging(${(user == 'admin') ? 'pListA' : 'pList'}, '${startPage-pages}', '${param.category}', '${param.keyword}')" />
						</td>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${(startPage+pages-1 < lastPage)? startPage+pages-1 : lastPage}">
						<td><input class="pageNo" type="button" value="${i}" name="${(pNo == i)?'currentPage':''}"
							onClick="paging(${(user == 'admin') ? 'pListA' : 'pList'}, '${i}', '${param.category}', '${param.keyword}')" />
						</td>
					</c:forEach>
